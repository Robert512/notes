![1588174881999](./Spring注解驱动开发.assets/1588174881999.png)
...
<beans xmlns="http://www.springframework.org/schema/beans"
	...
	<bean id="person" class="com.jerry.bean.Person">
...
```xml
	<!--只要标注了@Controller、@Service、@Repository、@Component就扫描到容器-->
...
```xml
  ...
  BeanPostProcessor原理
  populateBean(beanName, mbd, instanceWrapper); （构造器）给bean进行属性赋值
  initializeBean
  {
...
```xml
执行顺序：
生成beanDefination -> 构造器 -> 属性赋值
-> postProcessBeforeInitialization
->自定义初始化方法（init-method -> InitializingBean -> @PostConstruct）
...
![1588303261159](Spring注解驱动开发.assets/1588303261159.png)
...
![1588303053710](Spring注解驱动开发.assets/1588303053710.png)
...
3、registerBeanPostProcessors(beanFactory);注册bean的后置处理器来方便拦截bean的创建；
	1）、先获取ioc容器已经定义了的	需要创建对象的所有BeanPostProcessor
...
obtainFreshBeanFactory	获取子类刷新后内部的BeanFactory实例
...
onRefresh	在单例Bean初始化之前预留给子类初始化其他特殊的bean的口子
